...
import { Button, Chip, Dialog, DialogContent, Grid, Slider, Box, Typography } from "@mui/material";
              ...
              <Grid container>
                <Grid item md={6}>
                  ...
                  {pokemonData.abilities.map(abilitie => (
                    <Chip label={abilitie.ability.name} color="primary" sx={{ marginRight: 2 }} />
                  ))}
                  <h4>Datos</h4>
                  ...
                  {pokemonData.stats.map((stat) => (
                    <Box>
                      <Typography gutterBottom align="left" textTransform={"uppercase"}>{stat.stat.name}</Typography>
                      <Slider disabled defaultValue={+stat.base_stat} aria-label="Temperature" valueLabelDisplay="on" step={10} marks color={"success"} />
                    </Box>
                  ))}
                </Grid>
